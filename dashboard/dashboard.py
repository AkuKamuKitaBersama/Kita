...
def monthly_weekday_count(data):
    return data.groupby(['month', 'workingday']).agg({'total_count': ['mean', 'sum']}).reset_index()
...
def seasonal(data):
    return data.groupby(['date', 'season']).agg({'temp': 'mean','total_count': 'sum'}).reset_index()
...
with col2:
    ...
    """.format(casual_users), unsafe_allow_html=True)
...
with col3:
    ...
    """.format(registered_users), unsafe_allow_html=True)
...
with st.container():
    ...
    st.subheader('The Influence of Working Day and Weekend on Monthly Bike Sharing Rental Counts')
    st.markdown('<hr>', unsafe_allow_html=True)
...
with st.container():
    ...
    with hour1:
        ...
        plt.legend()
        plt.close(hourly_fig)  # Close the figure to prevent it from being displayed prematurely
...
tab1, tab2, tab3, tab4 = st.tabs(["Count", "Temperature", "Season", "daily/weekly trent"])
...
with tab1:
    ...
    with charT2:
        ...
        ax = sns.barplot(x='season', y='total_count', data=main_data, palette='Set2', errorbar=None)
...
with tab4:
    ...
    sns.lineplot(x='date', y='total_count', data=daily_data, color='skyblue', sort=False)
    ...
    sns.lineplot(x= 'date', y='total_count' ,data= weekly_average, color='blue', marker='o', sort=False)
